...
<?php 
    if (!current_user_can('manage_options')) {
        wp_die(printf('<div class="error fade"><p>%s</p></div>',
            __('You are not allowed to view the submission log.', WPCFORM_I18N_DOMAIN)));
    } 
 ...
 * 
 */
class wpCForms_List_Table extends WP_List_Table {
    ...
    function column_timestamp($item) {
		...
		//        get_admin_url(), 'user-edit.php?user_id=',$item->ID),
		    'delete' => sprintf('<a href="%s%s%s">Delete Log Entry</a>',
                get_admin_url(), 'edit.php?post_type=' . WPCFORM_CPT_FORM .
                '&page=wpcform-entry-log-page&action=delete&form=', $item->form),
        );
    ...
    function get_columns(){
        $columns = array(
            ...
            'timestamp'        => __('Timestamp', WPCFORM_I18N_DOMAIN),
            'url'              => __('URL', WPCFORM_I18N_DOMAIN),
            'remote_addr'      => __('Remote Addr', WPCFORM_I18N_DOMAIN),
            'remote_host'      => __('Remote Host', WPCFORM_I18N_DOMAIN),
            'http_referer'     => __('HTTP Refer', WPCFORM_I18N_DOMAIN),
            'http_user_agent'  => __('HTTP User Agent', WPCFORM_I18N_DOMAIN),
            //'form'             => __('wpCForm Id', WPCFORM_I18N_DOMAIN),
            //'post_id'          => __('Post Id', WPCFORM_I18N_DOMAIN)
        );
        return $columns;
    }
    ...
    function get_bulk_actions() {
        $actions = array(
            'delete' => __('Delete', WPCFORM_I18N_DOMAIN)
        );
        return $actions;
    }
    ...
    function process_bulk_action() {
        ...
        {
            /* -- Fetch the items -- */
            $raw_meta_values = $this->get_meta_key_values(WPCFORM_LOG_ENTRY_META_KEY, '', -1);
            ...
            {
                $meta_value = wpCFormArrayToObject(array_merge(maybe_unserialize($raw_meta_value->mv),
                ...
                switch ($this->current_action())
                {
                    case 'delete':
                        ...
                        delete_post_meta($meta_values[$form]->form,
                            WPCFORM_LOG_ENTRY_META_KEY, maybe_unserialize($meta_values[$form]->meta_value)) ;
                        break ;
                }
            }
        ...
        else if ($this->current_action() !== false)
        {
            printf('<div class="error fade"><h4>%s.</h4></div>',
                __('No log entries selected', WPCFORM_I18N_DOMAIN)) ;
        }
    ...
    function pagination($which) {
        $_SERVER['REQUEST_URI'] = add_query_arg(array(
            'post_type' => WPCFORM_CPT_FORM
           ,'page' => 'wpcform-entry-log-page'
        ), $_SERVER['REQUEST_URI']) ;
    ...
    function prepare_items() {
         ...
         */
        //$per_page = wpcform_get_user_settings_table_rows() ;
        $per_page = 10 ;
        ...
        $totalitems = $this->get_meta_key_count(WPCFORM_LOG_ENTRY_META_KEY);
        ...
        /* -- Fetch the items -- */
        $raw_meta_values = $this->get_meta_key_values(WPCFORM_LOG_ENTRY_META_KEY, $paged, $per_page);
        ...
        foreach ($raw_meta_values as $raw_meta_value)
            $this->items[] = wpCFormArrayToObject(array_merge(maybe_unserialize($raw_meta_value->mv),
                array('meta_value' => $raw_meta_value->mv))) ;
    }

    function get_meta_key_values($key = '', $paged = '', $per_page = 10, $type = 'wpcform', $status = 'publish')
    {
        global $wpdb;
     ...
     */
    function get_meta_key_count($key = '', $type = 'wpcform', $status = 'publish') {
        global $wpdb;
 ...
 */
function wpcform_render_list_page(){
    
    //Create an instance of our package class...
    $wpcformListTable = new wpCForms_List_Table();

    //Fetch, prepare, sort, and filter our data...
    $wpcformListTable->prepare_items();

    ?>
    <div class="wrap">
        
        <div id="icon-users" class="icon32"><br/></div>
        <h2>WordPress Chat Form Submission Log</h2>
        
        <!-- Forms are NOT created automatically, so you need to wrap the table in one to use features like bulk actions -->
        <form id="wpcform-log-entries-filter" method="get">
            <!-- For plugins, we also need to ensure that the form posts back to our current page -->
            <input type="hidden" name="post_type" value="<?php esc_attr_e( WPCFORM_CPT_FORM ); ?>" />
            <input type="hidden" name="page" value="<?php esc_attr_e($_GET['page']) ?>" />
            <input type="hidden" name="_wp_http_referer" value="<?php echo admin_url('edit.php?post_type=' . WPCFORM_CPT_FORM .  '&amp;page=wpcform-entry-log-page' ); ?>" />
            <!-- Now we can render the completed list table -->
            <?php //$wpcformListTable->search_box(__('Search', WPCFORM_I18N_DOMAIN), 'search_id'); ?>
            <?php $wpcformListTable->display() ; ?>
        </form>
 ...
 * @return stdObject
 */
function wpCFormArrayToObject($d)
{
    if (is_array($d)) {
        /*
